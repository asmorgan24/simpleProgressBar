...
Sub AddProgressBar()
    On Error Resume Next
        With ActivePresentation
              ...
              '
              'If the presentation only has a main section, just use a simple progress bar
              '
              If num_sects = 0 Then
                For i = 2 To .Slides.Count
                      If .Slides(i).SlideShowTransition.Hidden = msoFalse Then
                        'Loc = (i - n) * .PageSetup.SlideWidth / (.Slides.Count - j)
                        Set slider = .Slides(i).Shapes.AddShape(msoShapeRectangle, 0, vert_loc, (i - n) * .PageSetup.SlideWidth / (.Slides.Count - j), bar_height)
                        With slider
                        ...
                        Set pageNumber = .Slides(i).Shapes.AddTextbox(msoTextOrientationHorizontal, (i - n) * .PageSetup.SlideWidth / (.Slides.Count - j) - 50, vert_loc - 3, 100, bar_height - 2)
                        With pageNumber
              ...
              '
              'If the presentation has sections, use a new advanced style progress bar
              '
              Else
                  For i = 2 To .Slides.Count
                    If .Slides(i).SlideShowTransition.Hidden = msoFalse Then
